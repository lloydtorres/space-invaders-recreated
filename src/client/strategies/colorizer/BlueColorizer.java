...
public class BlueColorizer implements Colorizer{
    private final Color blueColor = new Color(0, 218, 255);
    private final Color colorToChange;
    private final int tolerance = 50;

    public BlueColorizer(Color colorToChange)
    {
            ...
            for (int x = 0; x < originalImage.getWidth(); x++) {
                for (int y = 0; y < originalImage.getHeight(); y++) {
                    ...
                    if (needsColoring(pixelColor)) {
                        recoloredImage.setRGB(x, y, blueColor.getRGB());
                    } else {
