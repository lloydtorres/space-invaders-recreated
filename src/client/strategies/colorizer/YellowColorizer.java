...
public class YellowColorizer implements Colorizer{
    private final Color yellowColor = new Color(247, 255, 0);
    private final Color colorToChange;
    private final int tolerance = 50;

    public YellowColorizer(Color colorToChange)
    {
            ...
            for (int x = 0; x < originalImage.getWidth(); x++) {
                for (int y = 0; y < originalImage.getHeight(); y++) {
                    ...
                    if (needsColoring(pixelColor)) {
                        recoloredImage.setRGB(x, y, yellowColor.getRGB());
                    } else {
