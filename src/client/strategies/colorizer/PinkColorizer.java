...
public class PinkColorizer implements Colorizer{
    private final Color pinkColor = new Color(255, 88, 223);
    private final Color colorToChange;
    private final int tolerance = 50;

    public PinkColorizer(Color colorToChange)
    {
            ...
            for (int x = 0; x < originalImage.getWidth(); x++) {
                for (int y = 0; y < originalImage.getHeight(); y++) {
                    ...
                    if (needsColoring(pixelColor)) {
                        recoloredImage.setRGB(x, y, pinkColor.getRGB());
                    } else {
